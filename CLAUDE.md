...
- `assets/icon.ico` is generated by `assets/generate_icon.py` and contains sizes: 16, 32, 48, 64, 128, 256px.
- The `.exe` is built via GitHub Actions (`.github/workflows/build.yml`) — trigger manually from the Actions tab. The artifact is available for download for 30 days. Do not build the `.exe` on macOS; PyInstaller only targets the OS it runs on.
- PyInstaller is configured with `--onedir` (not `--onefile`) to avoid antivirus false positives and allow execution without admin rights in corporate environments. The artifact is a folder; the user must keep `MouseJigglerSchedule.exe` alongside its dependency files.
...
3. Download `MouseJigglerSchedule-windows.zip` from the run's Artifacts section.
4. Test the `.exe` before publishing.

### Publishing a release

1. Test both builds.
2. Create a git tag: `git tag v1.x.x && git push origin v1.x.x`
3. Go to **GitHub → Releases → New release**, select the tag.
4. Attach `MouseJigglerSchedule.exe` and `MouseJigglerSchedule-mac.zip`.
5. Publish.
